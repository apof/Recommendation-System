#include "HyperCube.h"
	...
	list<MyVector*> Cube::range_search_cosine(MyVector* vec,double range){
    	...
    	if(index!=-1)
    	{
    		range_list.splice(range_list.end(),Table[index]->range_search_cosine(vec,range,neighbors_examined,M));
    	}
	...
	list<MyVector*> Cube::range_search_eucledian(MyVector* vec, double range){
    	...
    	while((neighbors_examined < M) || (probes_examined < probes) )
    	{
    	   if(index!=-1)
    	   {
    		 range_list.splice(range_list.end(),Table[index]->range_search_eucledian(vec,indx,range,neighbors_examined,M));
    	   }
