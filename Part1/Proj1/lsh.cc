...
int main(int argc, char** argv){
 ...
 if(argc!=15) { 
    cout<<"Wrong arguments, must be: -d <inputfile> -q <queryfile> -k <int> -L <int> -o <outputfile> -m <metric> -r <radius>"<<endl; 
    return 0; 
 }
 else
 {
    ...
    output_file = new char[strlen(argv[10]) + 1];
    strcpy(output_file,argv[10]);

    K=atoi(argv[6]);
    l=atoi(argv[8]);

    metric = new char[strlen(argv[12]) + 1];
    strcpy(metric,argv[12]);
...
cout<<"Reading input and Loading data to LSH Tables..."<<endl;
...
while (inputfile.getline(buffer, sizeof(buffer)))
{
    ...
    if(strcmp(metric,"eucledian")==0)
        Tables->insert_eucledian(vec_pointers[data_index]);
    else if(strcmp(metric,"cosine")==0)
        Tables->insert_cosine(vec_pointers[data_index]);
...
while (queryfile.getline(buffer, sizeof(buffer)))
{
    ...
    Tables->range_search_eucledian(query_vec,range,outfile);
    ...
    double ret2 = Tables->NN_search_eucledian(query_vec);
    outfile<<ret2<<endl;
    time3 = clock();
    ...
    Tables->range_search_cosine(query_vec,range,outfile);
    ...
    outfile<<"distance LSH: ";
    double ret2 = Tables->NN_search_cosine(query_vec);
    outfile<<ret2<<endl;
    time3 = clock();
